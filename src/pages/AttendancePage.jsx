...
import { clockIn, clockOut, deleteShift, fetchMyAttendance, fetchMyShifts, saveShift, saveShifts } from "../services/attendanceService";
...
export default function AttendancePage({ currentProfile, onGoLists, onLogout, onOpenAdmin, onOpenMyPage }) {
  ...
  async function bulkSave(dates, settings) { try { await saveShifts(userId, dates, settings); await reload(); } catch(e) { setError(e.message || "シフトの一括登録に失敗しました。"); throw e; } }
  async function singleSave(date, settings) { try { await saveShift({ userId, shiftDate: date, ...settings }); await reload(); } catch(e) { setError(e.message || "シフト保存に失敗しました。"); throw e; } }
  async function removeShift(shift) { try { await deleteShift(shift.id); await reload(); } catch(e) { setError(e.message || "シフト削除に失敗しました。"); throw e; } }
  ...
  return <main className="app-page">
    ...
    <section className="content">
      ...
      <section className="simple-panel"><div className="admin-panel-head"><div><h2>シフト登録・確認</h2><p>複数日を選んで一括登録し、日付ごとに個別時間も設定できます。</p></div><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></div>
        {loading ? <div className="empty-state">読み込み中...</div> : <ShiftCalendarEditor month={month} shifts={shifts} onBulkSave={bulkSave} onSingleSave={singleSave} onDelete={removeShift} />}
      </section>
