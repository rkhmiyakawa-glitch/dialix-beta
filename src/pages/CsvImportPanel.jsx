  ...
  return <div className="csv-import-grid">
    <section className="admin-panel csv-panel">
      ...
      {headers.length > 0 && <>
        ...
        {rawStatuses.length > 0 && <><h3 className="csv-subtitle">ステータスマッピング</h3><p className="csv-note">スプレッドシート側の表記をDIALIXのステータスへ割り当ててください。「使用しない」を選ぶと、未架電として取り込みます。未設定の値がある行は取り込みません。</p><div className="mapping-grid status-mapping-grid">{rawStatuses.map((value) => <label key={value}><span>{value}</span><select value={statusMappings[value] || ""} onChange={(e) => setStatusMappings({ ...statusMappings, [value]: e.target.value })}><option value="">選択してください</option><option value={IGNORE_STATUS_MAPPING}>使用しない</option>{DIALIX_STATUSES.filter(Boolean).map((status) => <option key={status} value={status}>{status}</option>)}</select></label>)}</div></>}
