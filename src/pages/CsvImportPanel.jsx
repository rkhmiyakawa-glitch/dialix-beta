  ...
  return <div className="csv-import-grid">
    <section className="admin-panel csv-panel">
      ...
      {headers.length > 0 && <>
        ...
        {rawStatuses.length > 0 && <><h3 className="csv-subtitle">ステータスマッピング</h3><p className="csv-note">スプレッドシート側の表記をDIALIXのステータスへ割り当ててください。「使用しない」を選ぶと、ステータスを空欄にして未架電として取り込みます。未設定の値がある行は取り込みません。</p><div className="mapping-grid status-mapping-grid">{rawStatuses.map((value) => <label key={value}><span>{value}</span><select value={statusMappings[value] || ""} onChange={(e) => setStatusMappings({ ...statusMappings, [value]: e.target.value })}><option value="">選択してください</option><option value={IGNORE_STATUS_MAPPING}>使用しない（未架電として取込）</option>{DIALIX_STATUSES.filter(Boolean).map((status) => <option key={status} value={status}>{status}</option>)}</select></label>)}</div></>}

        {rawApNames.length > 0 && <><h3 className="csv-subtitle">最終担当APマッピング</h3><p className="csv-note">該当ユーザーがいない場合は「名前だけ履歴に残す」、AP情報を取り込まない場合は「使用しない」を選択できます。</p><div className="mapping-grid status-mapping-grid">{rawApNames.map((name) => <label key={name}><span>{name}</span><select value={apMappings[name] === IGNORE_AP_MAPPING ? IGNORE_AP_MAPPING : (apMappings[name]?.id || "")} onChange={(e) => { const selected = e.target.value; const nextValue = selected === IGNORE_AP_MAPPING ? IGNORE_AP_MAPPING : (profiles.find((p) => p.id === selected) || null); setApMappings({ ...apMappings, [name]: nextValue }); }}><option value="">名前だけ履歴に残す</option><option value={IGNORE_AP_MAPPING}>使用しない</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.displayName}{profile.isActive ? "" : "（停止中）"}</option>)}</select></label>)}</div></>}
